...
// Tells the UI whether the admin token has been set up yet, so it can show either the
// first-time setup screen or the login screen.
// First-login bootstrap: generate the admin token, store its hash, return the plaintext once.
// Verifies a token so the UI can unlock the panel. The admin must do this every time they open
// the panel — there is no persistent session.
...
async fn verify_admin_handler(
    ...
    let mut conn = match guard_mysql_database_pool.get_conn().await {
        Ok(c) => c,
        Err(_) => {
            return (
                axum::http::StatusCode::INTERNAL_SERVER_ERROR,
