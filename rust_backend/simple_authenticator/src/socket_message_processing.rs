...
pub async fn socket_message_processing() {
    ...
    loop {
        let (client, _) = guard_simple_authenticator_socket.accept().await.unwrap();
            ...
            if let Ok(msg) = msg {
                if msg.get("ws_id").is_some() {
                    ...
                    if let Ok(msg) = serde_json::from_value::<SocketJsonMessageWithWsId>(msg) {
                        ...
                        } else if msg.r#type == "on_session_restore" {
                            socket_actions::on_session_restore::on_session_restore(msg).await;
                        } else {
                            println!(
                                "{}",
